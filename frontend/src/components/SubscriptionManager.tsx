...
export default function SubscriptionManager({ externalLoading = false }: SubscriptionManagerProps) {
  ...
  const handlePaymentCreated = (confirmationUrl: string) => {
    ...
    // После оплаты браузер автоматически закроется, возвращая пользователя в Mini App
    console.log('Opening payment in Telegram internal browser...')
    
    const webApp = window.Telegram?.WebApp
    if (webApp && webApp.openLink) {
      // Telegram встроенный браузер - откроется как overlay, автоматически закроется после оплаты
      webApp.openLink(confirmationUrl)
      
      // Начинаем периодически проверять статус платежа
      ...
      // Останавливаем проверку через 10 минут (если пользователь не вернулся)
      setTimeout(() => {
        clearInterval(checkInterval)
      }, 10 * 60 * 1000)
    } else {
